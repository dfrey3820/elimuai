...
from ..services.ch6_commission_engine import calculate_ch6_commission

router = APIRouter(tags=["ch6-insurance-network"])
...
@router.post("/api/admin/ch6/networks", dependencies=[AdminOnly])
...
@router.get("/api/admin/ch6/csv-template", dependencies=[AdminOnly])
...
# ─── Admin: bulk import ────────────────────────────────────────────────────
@router.post("/api/admin/ch6/bulk-import", dependencies=[AdminOnly])
async def bulk_import_agents(
    ...
    employee_to_partner: dict[str, str] = {}

    # PASS 1: Managers
    staged = await sess.execute(text("""
    ...
    for row in staged.fetchall():
        try:
            ...
            if not manager_partner_id:
                existing_mgr = await sess.execute(
...
@router.get("/api/admin/ch6/bulk-import/{batch_id}/errors", dependencies=[AdminOnly])
async def get_import_errors(batch_id: str, sess: AsyncSession = Depends(get_session)):
...
# ─── Webhook: process a conversion event (called by billing service) ───────
@router.post("/api/ch6/conversions/process")
async def process_ch6_conversion(
    payload: CH6ConversionRequest,
    sess: AsyncSession = Depends(get_session),
):
    """
    ...
    """
    agent_result = await sess.execute(text("""
    ...
    if agent.parent_partner_id:
        mgr_result = await sess.execute(text("""
...
# ─── Admin dashboards ──────────────────────────────────────────────────────
@router.get("/api/admin/ch6/networks", dependencies=[AdminOnly])
async def list_networks(sess: AsyncSession = Depends(get_session)):
    result = await sess.execute(text("SELECT * FROM v_ch6_network_performance"))
    return [
        {
...
@router.get("/api/admin/ch6/networks/{network_id}/managers", dependencies=[AdminOnly])
async def list_managers(network_id: str, sess: AsyncSession = Depends(get_session)):
    result = await sess.execute(text("""
...
@router.get("/api/admin/ch6/managers/{manager_id}/agents", dependencies=[AdminOnly])
async def list_agents_for_manager(manager_id: str, sess: AsyncSession = Depends(get_session)):
    result = await sess.execute(text("""
...
@router.get("/api/admin/ch6/agents/{agent_id}", dependencies=[AdminOnly])
async def get_agent_detail(agent_id: str, sess: AsyncSession = Depends(get_session)):
    result = await sess.execute(
        text("SELECT * FROM v_ch6_agent_performance WHERE agent_id = :id"),
        {"id": agent_id},
    )
    r = result.fetchone()
    if not r:
