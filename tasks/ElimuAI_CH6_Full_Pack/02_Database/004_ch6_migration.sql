...
-- PostgreSQL Migration — run after 001_migration.sql and 002_compliance_migration.sql
-- ============================================================

-- Extend channel_type enum with the new channel
...
ALTER TABLE partners ADD COLUMN IF NOT EXISTS network_id UUID;          -- groups an entire network under one head
ALTER TABLE partners ADD COLUMN IF NOT EXISTS team_code VARCHAR(50);    -- manager's team identifier, e.g. "TEAM-NBI-04"
ALTER TABLE partners ADD COLUMN IF NOT EXISTS employee_number VARCHAR(50); -- insurance company staff/agent number
ALTER TABLE partners ADD COLUMN IF NOT EXISTS insurance_company VARCHAR(255); -- e.g. "Jubilee Insurance"
...
-- ============================================================
-- Insurance Networks table — one row per partner network (e.g. your friend's network)
-- ============================================================
CREATE TABLE insurance_networks (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  network_name      VARCHAR(255) NOT NULL,           -- e.g. "Acme Insurance Agency Network"
  head_partner_id   UUID NOT NULL REFERENCES partners(id),  -- the "friend" / head
...
-- Link partners.network_id to this table conceptually (no FK constraint to allow flexible grouping)

-- ============================================================
-- Commission rate config for CH6 (continuous, no diminishing)
-- Stored in DB so rates can be adjusted without code changes
-- ============================================================
CREATE TABLE ch6_commission_rates (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  role        VARCHAR(30) NOT NULL UNIQUE,  -- 'agent', 'manager', 'network_head', 'gm_rider'
...
-- Used for CSV uploads of the 1,200 agents
-- ============================================================
CREATE TABLE ch6_bulk_import_staging (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  email             VARCHAR(255),
  role              VARCHAR(30) NOT NULL,        -- 'agent', 'manager', 'network_head'
  manager_employee_number VARCHAR(50),           -- links agent to their manager via employee number
  employee_number   VARCHAR(50) NOT NULL,
  region            VARCHAR(100),
  insurance_company VARCHAR(255),
  custom_code       VARCHAR(50),                 -- optional pre-assigned referral code
  status            VARCHAR(20) NOT NULL DEFAULT 'pending', -- pending, created, error
  error_message     TEXT,
  partner_id        UUID REFERENCES partners(id),
  created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_staging_batch ON ch6_bulk_import_staging(import_batch_id);
CREATE INDEX idx_staging_status ON ch6_bulk_import_staging(status);
CREATE INDEX idx_staging_employee ON ch6_bulk_import_staging(employee_number);

-- ============================================================
-- View: Network performance rollup
-- ============================================================
CREATE VIEW v_ch6_network_performance AS
SELECT
...
-- ============================================================
CREATE VIEW v_ch6_manager_teams AS
SELECT
...
-- ============================================================
CREATE VIEW v_ch6_agent_performance AS
SELECT
  agt.id AS agent_id,
