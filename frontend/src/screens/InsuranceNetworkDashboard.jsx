...
// src/screens/InsuranceNetworkDashboard.jsx
//
...
export default function InsuranceNetworkDashboard({ role = "network_head", partnerId, networkId }) {
  ...
  const fetchManagers = useCallback(async (nid) => {
    setManagers(await apiGet(`/api/admin/ch6/networks/${nid}/managers`));
  }, []);

  const fetchAgents = useCallback(async (mgrId) => {
    setAgents(await apiGet(`/api/admin/ch6/managers/${mgrId}/agents`));
  }, []);

  const fetchAgentDetail = useCallback(async (id) => {
    setAgentDetail(await apiGet(`/api/admin/ch6/agents/${id}`));
  }, []);
  ...
  if (err) return (
    <div style={{ padding: 24, color: "#c00", fontSize: 14 }}>Error: {err}</div>
  );
          ...
          <Table
            ...
            rows={agents.map(a => ({
              cells: [
                ...
                <code key="c" style={{ background: "#f0f4ff", padding: "2px 6px", borderRadius: 4 }}>{a.referral_code}</code>,
                a.total_families_enrolled, fmt(a.pending_kes), fmt(a.paid_kes),
                <span key="s" style={{ color: a.status === "active" ? GREEN : "#c00", fontWeight: 600 }}>{a.status}</span>
              ],
              _raw: a
            }))}
