            ...
            {userDetailTab === "admin" && (<div className="space-y-5">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="p-5">
                  ...
                  <div className={`grid gap-3 ${isMobile ? "grid-cols-1" : "grid-cols-3"}`}>
                    <div><p className="text-slate-400 text-[10px] font-body font-bold mb-1 mt-0">Plan</p><select value={upgradeForm.plan} onChange={(e) => setUpgradeForm((p) => ({ ...p, plan: e.target.value }))} className={inputCls}>{["free", "student", "parent", "teacher", "family", "school", "enterprise"].map((p) => (<option key={p} value={p}>{p}</option>))}</select></div>
